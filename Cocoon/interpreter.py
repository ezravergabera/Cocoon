...
class Interpreter:
    ...
    def visit_BuildDefNode(self, node, context):
        ...
        func_value = Function(func_name, body_node, arg_names, node.should_return).set_context(context).set_pos(node.pos_start, node.pos_end)
...
class Function(BaseFunction):
    def __init__(self, name, body_node, arg_names, should_return):
        super().__init__(name)
    ...
    def execute(self, args):
        ...
        if res.should_return() and res.should_return == None: return res

        ret_value = (value if self.should_return else None) or res.return_value or Number.empty
        return res.success(ret_value)
    
    def copy(self):
        copy = Function(self.name, self.body_node, self.arg_names, self.should_return)
        copy.set_context(self.context)
        copy.set_pos(self.pos_start, self.pos_end)
        return copy
